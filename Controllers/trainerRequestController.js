...
exports.acceptTrainerRequest = async (req, res) => {
  ...
  try {
    const result = await trainerRequestModel.findByIdAndUpdate(
      requestId,
      { status: "accepted" },
      { new: true },
    );
...
exports.declineTrainerRequest = async (req, res) => {
  ...
  console.log(req.body);
  try {
    const result = await trainerRequestModel.findByIdAndDelete(
      requestId,
      { status: "rejected" },
      { new: true },
    );
...
exports.viewMyClients = async (req, res) => {
  ...
  try {
    const result = await trainerRequestModel
      .find({
        trainer: trainerId,
        status: "accepted",
      })
...
exports.viewMyTrainer = async (req, res) => {
  const userId = req.id;
  console.log(userId);
  try {
    const trainer = await trainerRequestModel
      .find({
        user: userId,
        status: "accepted",
      })
      .populate("trainer", "username bio profile");
